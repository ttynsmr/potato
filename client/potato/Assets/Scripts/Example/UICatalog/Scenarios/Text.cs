...
namespace UICatalog.Scenarios {
	...
	public class Text : Scenario {
		public override void Setup ()
		{
			...
			{
				textField.Autocomplete.AllSuggestions = Regex.Matches (e.NewText.ToString (), "\\w+").Cast<Match>()
                    .Select (s => s.Value)
					.Distinct ().ToList ();
			}
			Win.Add (textField);
			...
			{
				textView.Autocomplete.AllSuggestions = Regex.Matches (textView.Text.ToString (), "\\w+").Cast<Match>()
                    .Select (s => s.Value)
					.Distinct ().ToList ();
			}
			Win.Add (textView);
