		...
		unitSpawnReady->subscribeRequest([this, session](const auto& request, auto& responser)
			{
				...
				// response
				{
					...
					auto position = new torikime::Vector3();
					position->set_x(0);
					position->set_y(0);
					position->set_z(0);
					response.set_allocated_position(new torikime::Vector3());
					response.set_direction(0);
					auto individuality = new torikime::Individuality();
					individuality->set_type(torikime::UNIT_TYPE_PLAYER);
					response.set_allocated_individuality(individuality);
					response.set_cause(torikime::UNIT_SPAWN_CAUSE_LOGGEDIN);
					auto avatar = new torikime::Avatar();
...

				{
					...
					position->set_y(0);
					position->set_z(0);
					notification.set_allocated_position(new torikime::Vector3());
					notification.set_direction(0);
					auto individuality = new torikime::Individuality();
					individuality->set_type(torikime::UNIT_TYPE_PLAYER);
					notification.set_allocated_individuality(individuality);
					notification.set_cause(torikime::UNIT_SPAWN_CAUSE_LOGGEDIN);
					auto avatar = new torikime::Avatar();
					avatar->set_name(std::string("hoge"));
					...
					for (auto unit : units)
					{
						...
						// spawn
						{
							...
							individuality->set_type(torikime::UNIT_TYPE_PLAYER);
							notification.set_allocated_individuality(individuality);
							notification.set_cause(torikime::UNIT_SPAWN_CAUSE_ASIS);
							auto avatar = new torikime::Avatar();
							avatar->set_name(std::string("hoge"));
							notification.set_allocated_avatar(avatar);
